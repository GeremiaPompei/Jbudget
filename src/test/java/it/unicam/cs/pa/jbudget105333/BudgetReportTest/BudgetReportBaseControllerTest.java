...
import it.unicam.cs.pa.jbudget105333.Ledger.LedgerReaderTxt;
...
class BudgetReportBaseControllerTest {
    ...
    void createBudgetReportBaseController(){
        try {
            new ObjectOutputStream(new FileOutputStream("src/file/Ledger"));
            new ObjectOutputStream(new FileOutputStream("src/file/Budget"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    */
    ...
    void save() {
        try {
            ...
            this.budRepC.save();
            Reader<Ledger> reader1 = new LedgerReaderTxt("src/file/Ledger");
            Ledger ledger = reader1.read();
            reader1.close();
            Reader<Budget> reader2 = new BudgetReader("src/file/Budget");
            Budget budget = reader2.read();
            reader2.close();
            assertEquals(this.budRepC.getLedgerC().getTag(sport.getID()).getName(),ledger.getTags().iterator()
                    .next().getName());
            assertEquals(this.budRepC.getLedgerC().getTag(sport.getID()).getID(),ledger.getTags().iterator()
                    .next().getID());
